...
class woocsvExportProduct
{
	...
	public function getHeader() {
		...
		$all_fields = 	array_merge($this->body,$this->meta);
		$header = array ();
	...
	public function getProduct() {
		...
		$products[] = array_merge($this->body,$this->tags,$this->meta,$this->category,$this->attributes,$this->attribute_values,$this->extra);
	...
	public function fillBody(){
		...
		$this->body = $wpdb->get_row(
			$wpdb->prepare( "SELECT post_parent, ID, '', 'BLB', post_title, 'black', 'compact' FROM $wpdb->posts WHERE ID = %d",$this->ID),ARRAY_A
		);
		...
		$this->body['post_parent'] = $this->ID;
		$this->body['quantity'] = '30';
	...
	public function fillMeta() {
		...
		//$this->meta['_stock'] = get_post_meta($this->ID, '_stock' ,true);
		$this->meta['_description'] = get_post($this->ID)->post_content;
		$this->meta['_msrp'] = '';
		$this->meta['_price'] = get_post_meta($this->ID, '_price' ,true);
		$this->meta['_shipping'] = '0';
		$this->meta['_product_url'] = '';
		//loop through meta
		/*foreach ($allmeta as $key=>$value) {
			//if key in field lists add it to row
			if ( array_key_exists($key, $this->meta) ) {
				if ($key == '_price') {
					$this->meta[$key] = $key.' - '.$value[0];
				}
				if ($key == '_stock') {
					$this->meta[$key] = $key.' - '.$value[0];
				}

				if ($key !='_manage_stock' || $key !='_sku') {
					//$this->meta[$key] = $key.' - '.$value[0];
				}
			}
		}*/
